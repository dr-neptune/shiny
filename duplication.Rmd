...
```{r}
selectInMam <- function(id, label, selected = "sleep_rem") {
    ns <- NS(id)
    choices <-  c("sleep_total", "sleep_rem", "sleep_cycle", 
                  "awake", "brainwt", "bodywt")
...
filter_plot <- function(input, output, session, type) {
    data <- reactive(filter(msleep, vore == type))
...
ui <- fluidPage(
    selectInMam("x", "x-axis"),
    selectInMam("y", "y-axis", selected = "sleep_total"),
    tabsetPanel(id = "vore",
                tabPanel("Carnivore",
                         plotOutput("plot_carni")),
                tabPanel("Omnivore",
...
server <- function(input, output, session) {
    output$plot_carni <- callModule(filter_plot, "mammal_plot1", type = "carni")
    output$plot_omni <- callModule(filter_plot, "mammal_plot2", type = "omni")
    output$plot_herbi <- callModule(filter_plot, "mammal_plot3", type = "herbi")
}

sp()
```

<!-- STILL BROKEN, FIX THIS^^^ -->


```{r}
selectInputMammal <- function(id) {
    ns <- NS(id)

    choices <- c("sleep_total", "sleep_rem", "sleep_cycle",
                 "awake", "brainwt", "bodywt")
    
    tagList(selectInput(ns("x_val"),
                label = "x axis",
                choices = choices,
                selected = "sleep_total"),
    selectInput(ns("y_val"),
                label = "y axis",
                choices = choices,
                selected = "sleep_rem"),
...
ui <- fluidPage(
   selectInputMammal("mammals")
)

filterInputMammal <- function(input, output, server) {
    df <- reactive({msleep %>%
                        filter(vore == input$vore)})

    output$plot <- renderPlot({
        df() %>%
            plotter(input$x_val, input$y_val)})
}

server <- function(input, output, session) {
    data_filtered <- callModule(filterInputMammal, "mammals")
}
...
```{r}
ui <- fluidPage(
    selectInputMammal("mammal"))

serverMammal <- function(input, output, session) {
    df <- reactive({
        filter(msleep, vore == input$vore)
    })
...
server <- function(input, output, session) {
    df <- callModule(serverMammal, "mammal")

    output$plot_carni <- renderPlot({df() %>%
        plotter(input$x_val, input$y_val)})
}
...
mod_server <- function(input, output, session) {
    ds <- reactive(filter(msleep, vore == input$vore))

    output$plot <- df() %>% ggplot(aes_string(input$x, input$y)) + geom_point()
}

ui <- fluidPage(mod_ui("mammals"),
                tabsetPanel(id = "vore",
                            tabPanel("carni",
                                     plotOutput("plot")),
                            tabPanel("omni",
